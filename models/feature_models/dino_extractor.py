...
"""Code adapted from https://github.com/ShirAmir/dino-vit-features"""
...
class DINOExtractor:
    ...
    def __init__(
        ...
        if model is not None:
            ...
            self.model = DINOExtractor.create_model(model_type)
        ...
        self.model = DINOExtractor.patch_vit_resolution(self.model, stride=stride)
        self.model.eval()
        ...
        self.facet = "token" if "dinov2" in self.model_type else "key"
        self.layer = 11
    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        :return: the model
        """
        model_type = model_type.replace("sd_", "")
    ...
    def patch_vit_resolution(model: nn.Module, stride: int) -> nn.Module:
        ...
        model.interpolate_pos_encoding = types.MethodType(DINOExtractor._fix_pos_enc(patch_size, stride), model)
        return model
    ...
    def __call__(self, img):
        ...
        if "dinov2" in self.model_type:
            self.extract_descriptors(img, facet=self.facet, layer=self.layer, bin=False)
            features = self.model.forward_features(img)['x_norm_patchtokens'].unsqueeze(1)
        elif "dino" in self.model_type:
            features = self.extract_descriptors(img, facet=self.facet, layer=self.layer, bin=False)
        else:
